...
def test_visualizer_run_completes_series_headless(tmp_path):
    ...
    v = viz_mod.Visualizer(cfg, headless=True, fps=1000)
    totals = v.run()
    assert totals["cop"] + totals["thief"] > 0


def test_visualizer_run_records_gif_no_pngs(tmp_path):
    cfg = small_config(tmp_path)
    v = viz_mod.Visualizer(cfg, headless=True, fps=1000, record_gif=True)
    v.run()
    saved = os.listdir(cfg.q_dir_abs())
    # No game-screenshot PNGs are produced; the GIF is the report evidence.
    assert not [f for f in saved if f.startswith("gui_") and f.endswith(".png")]
...
def test_main_headless_cli_runs(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(sys, "argv", ["visualizer.py", "--headless", "--gif"])
    viz_mod.main()
